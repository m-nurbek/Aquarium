...
public class Fish extends DrawableImage {
	...
	public Fish(DrawingPanel drawArea, String imageFile, int speed, int speed_y,boolean facingRight) {
...
/* 		if (this.movingRight) {
			if (nextX > this.getWindowWidth()) {
				nextX = 0 - this.getWidth();
			}
			nextX += this.speed;
		} else {
			if (nextX <= 0-getWidth()) {
				nextX = this.getWindowWidth();
			}
			nextX -= this.speed;
		} */
		if (nextX > this.getWindowWidth() - this.getWidth() || nextX < 0) {
			reverseDirection();
		}
		if (this.movingRight)
			nextX += this.speed;
		...
		// Move in the y direction and bounce between the screen edges
		if (nextY > this.getWindowHeight() - this.getHeight() || nextY < 0 ) {
			this.speed_y *= (-1);
		}
		nextY += speed_y;
